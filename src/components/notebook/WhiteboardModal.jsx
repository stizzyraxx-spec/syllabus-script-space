...
const NOTE_W = 200;
const NOTE_H = 200;
const TACK_COLORS = ["#c0392b", "#2980b9", "#27ae60", "#8e44ad", "#d35400", "#16a085"];
...
function StickyNote({ note, onChange, onDelete, onBringForward, boardRef }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="absolute inset-0 pointer-events-none" style={{ background: "linear-gradient(135deg, rgba(255,255,255,0.4) 0%, rgba(0,0,0,0.05) 100%)", mixBlendMode: "soft-light" }} />
        <button
...
export default function WhiteboardModal({ userEmail, onClose }) {
  ...
  const addNote = (color) => {
    ...
    const tackColor = TACK_COLORS[Math.floor(Math.random() * TACK_COLORS.length)];
    setMaxZ(z => z + 1);
    setNotes(prev => [...prev, {
  ...
  const clearAll = () => {
    ...
    if (window.confirm("Remove all sticky notes from the corkboard?")) setNotes([]);
  };
    ...
    >
      <div className="w-full h-[92vh] max-w-7xl bg-card border border-border rounded-xl overflow-hidden flex flex-col">
            ...
            <div className="flex items-center gap-1 mr-2">
              {STICKY_COLORS.map(c => (
                <button key={c.id} onClick={() => addNote(c.id)}
                  ...
                  style={{ background: c.bg }} title={`Add ${c.id} note`} />
              ))}
            </div>
        ...
        <div ref={boardRef} className="flex-1 relative overflow-hidden"
          style={{ background: corkBg, backgroundSize: "60px 60px, 80px 80px, 100px 100px, 100% 100%", boxShadow: "inset 0 0 60px rgba(0,0,0,0.35)" }}>
          {notes.length === 0 && (
          ...
          {notes.map(n => (
            <StickyNote key={n.id} note={n} onChange={updateNote} onDelete={deleteNote} onBringForward={bringForward} boardRef={boardRef} />
          ))}
        </div>
      </div>
    </motion.div>
